...
export function ProcessObservationCard({ process, productId }: Props) {
  ...
  return (
    <Card
      className="p-4 space-y-3 max-w-full overflow-hidden"
    ...
    >
      <div className="flex items-start gap-3 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-sm break-words">{process.name}</h4>
          {process.description && (
            <p className="text-sm text-muted-foreground mt-1 break-words">{process.description}</p>
          )}
        </div>
