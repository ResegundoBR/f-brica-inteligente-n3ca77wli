...
export default function CotacoesPage() {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === cotacaoItems.length) {
  ...
  const handleCopySelected = () => {
    const items = cotacaoItems.filter((i) => selectedIds.has(i.id))
            ...
            <TableHeader className="bg-slate-50 dark:bg-slate-800/50">
              <TableRow>
                <TableHead className="w-[40px]">
                  <Checkbox
                    checked={selectedIds.size === cotacaoItems.length && cotacaoItems.length > 0}
