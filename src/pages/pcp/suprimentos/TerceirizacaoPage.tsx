...
import { PcpOrder, OutsourcingData } from '@/types'
...
export default function TerceirizacaoPage() {
  ...
  const handleConfirmReceipt = async (op: PcpOrder) => {
    try {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-8 bg-slate-50 min-h-[calc(100vh-4rem)] dark:bg-slate-950">
      <SuprimentosHeader
        title="Terceirização"
        description="Acompanhe serviços e materiais enviados a fornecedores externos."
        icon={Truck}
      />
      ...
      {outsourcedOrders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {outsourcedOrders.map((op) => {
            ...
            const outsourcingData: OutsourcingData[] = Array.isArray(op.outsourcing_data)
              ? op.outsourcing_data
              : []
            return (
              <Card
              ...
              >
                <CardHeader className="pb-2 flex flex-row items-start justify-between">
                  ...
                  <Badge variant="outline">{op.expand?.client_id?.name || op.client_name}</Badge>
                </CardHeader>
                <CardContent className="pb-3 text-sm space-y-3">
                  <div className="flex justify-between items-center text-xs bg-slate-50 dark:bg-slate-800/50 p-2 rounded border">
                    ...
                    >
                      {op.delivery_date ? format(new Date(op.delivery_date), 'dd/MM/yyyy') : '-'}
                    </span>
                  </div>
                ...
                <CardFooter>
                  <Button
                    ...
                    onClick={() => handleConfirmReceipt(op)}
                  >
